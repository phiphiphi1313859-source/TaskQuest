...
TaskQuest is a gamification system for Taskwarrior that adds D&D-style RPG mechanics to your task management. Complete tasks to earn XP, level up your character, collect loot, unlock achievements, and purchase rewards.
...
1. [Installation](#installation)
   ...
   - [Syncing Between Devices](#syncing-between-devices-with-syncthing)
2. [Initial Setup](#initial-setup)
3. [How TaskQuest Works](#how-taskquest-works)
4. [Character System](#character-system)
5. [Progression Mechanics](#progression-mechanics)
6. [Loot & Rewards](#loot--rewards)
7. [Achievements](#achievements)
8. [Commands Reference](#commands-reference)
9. [Taskwarrior Integration](#taskwarrior-integration)
...
1. Clone or download the TaskQuest repository:
   ```bash
   cd ~/Downloads  # or wherever you keep source code
...
Termux allows you to run Linux tools on Android. This is perfect for having TaskQuest on-the-go!
...
**Note**: On Termux, the default shell is usually already configured correctly, but verify `~/.cargo/bin` is in your PATH:
```bash
...
### Syncing Between Devices with Syncthing

Syncing your TaskQuest data between Linux and Termux allows you to maintain a single character across devices.

#### What to Sync

TaskQuest stores all data in `~/.taskquest/` and hooks in `~/.task/hooks/`. Here's what each file does:
...
**~/.task/ directory:**
- `pending.data` - Taskwarrior's pending tasks
- `completed.data` - Taskwarrior's completed tasks
- `undo.data` - Taskwarrior's undo history
- `.taskrc` - Taskwarrior configuration (includes UDAs)
- `hooks/on-add-taskquest` - Hook for adding tasks
- `hooks/on-modify-taskquest` - Hook for task completion (awards XP, gold, loot)
- `hooks/on-exit-taskquest` - Post-execution hook
...
2. **Start Syncthing** on both devices:
   ...
   Access the web UI at `http://localhost:8384` (Linux) or displayed IP (Termux)

3. **Add devices** - Pair your Linux machine and Termux device using device IDs

4. **Create folders to sync**:

   **Option A: Sync everything (recommended)**
   - Create a folder that syncs `~/.taskquest/` between devices
   - Create a folder that syncs `~/.task/` between devices

   **Option B: Selective sync (more control)**
   - Sync only `~/.taskquest/character.json`
   - Sync only `~/.taskquest/achievements.json`
   - Sync only `~/.taskquest/shop.json`
   - Sync only `~/.task/pending.data` and `~/.task/completed.data`

5. **Important Syncing Notes**:

   ⚠️ **Conflict Warning**: If you complete tasks on both devices while offline, you may get conflicts when they sync. Syncthing will create `.sync-conflict` files.

   **Best Practices**:
   - Always sync before using TaskQuest on a device
   - Avoid using both devices offline simultaneously
   - If conflicts occur, manually merge the data or keep the newest version

   **Hook Files**: The hooks (`~/.task/hooks/*`) need to be executable. After syncing, ensure they have execute permissions:
   ```bash
...
### Git-Based Sync (Recommended Alternative)

**New in Phase 3**: TaskQuest now supports git-based synchronization, which offers several advantages over Syncthing:

#### Advantages of Git Sync

✅ **Version Control** - Full history of character progression
...
✅ **Technical User-Friendly** - Fits CLI-comfortable users

See [SYNC_ANALYSIS.md](./SYNC_ANALYSIS.md) for detailed comparison.
...
2. **Create GitHub private repository** (one-time):
   ```bash
   # On GitHub: Create new private repository 'taskquest-data'
   # Then add as remote:
   cd ~/.taskquest
...
4. **On other devices** (after setup):
   ```bash
...
- `taskquest sync history` - View commit history
...
**Important**: Classes are purely for roleplay flavor. Your stats are determined by the tasks you complete, not your class!
...
- Each level makes you feel more powerful (though doesn't change rewards)
...
- Cost deducted
- Remaining gold
- Any achievements unlocked (e.g., "Wise Spender" for 10 purchases)
...
3. If no backup, recreate character:
   ```bash
...
While the shop currently has 10 default rewards, you can track your own rewards mentally or modify the shop JSON manually.

**Manual editing** (advanced):
```bash
nano ~/.taskquest/shop.json
```

Add custom rewards following the JSON structure. Be careful with formatting!
...
Track your stat growth over time by comparing backups.

### Achievement Progress Tracking

Check specific achievement progress by viewing the achievements file:
```bash
...
You can maintain multiple characters by swapping the `~/.taskquest/` directory:
...
├── .taskrc                 # Taskwarrior config (includes challenge UDA)
└── hooks/
...
# Restore from backup
cp -r ~/.taskquest_backup_20250119 ~/.taskquest
```
...
- Custom avatar display in status
- More achievement categories
- Reward shop customization UI
